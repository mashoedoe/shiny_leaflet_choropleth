...
#    gis$tj <- ward_tj_no_lines
    ...
    observe(if ((input$select_map_level == 'Province'|
                ...
                (!is.null(gis$slice))) {
        label = "town_province_proxymap_event"
        proxy <- leafletProxy(
            mapId = "map1", data = gis$tj
        )
        proxy %>%
            clearShapes() %>%
            clearGroup(group = 'proxy2') %>%
            addTopoJSON(
                layerId = gis$tj@data$ID, group = 'proxy1',
                topojson = gis$tj,
    ...
    observe(if (input$select_map_level == 'Ward') {
        ...
        proxy %>%
            clearShapes() %>%
            clearGroup(group = 'proxy1') %>%
            addPolygons(
                layerId = gis$tj@data$ID,
                stroke=T,dashArray=3,weight=2,color="white",
                opacity=1,fill=T,smoothFactor = 0.5, fillOpacity = 0.7,
                fillColor = gis$binpal(gis$shp@data$DENSITY)
            )
